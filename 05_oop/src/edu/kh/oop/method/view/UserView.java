...
public class UserView {
	...
	public void login() {
		...
		// 로그인이 되어 있지 않은 경우 -> 아이디, 비밀번호 입력받기
		if (loginUser != null) {
			...
			if (result == -1) {
				System.out.println("회원가입후 실행해주세요.");
			} else if (result == 0) {
				System.out.println("아이디 또는 비밀번호가 일치하지 않습니다.");
			} else { // 1
				System.out.println("[Login Success]");
				loginUser = user; // user변수가 참조하는 객체의 주소를 loginUser에 대입 (얕은 복사)
			}
	...
	public void printUser() {
		...
		if (loginUser != null) { // 로그인 0
			...
			System.out.println("성별 : " + loginUser.getUserGender());
